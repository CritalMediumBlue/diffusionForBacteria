...
The Crank‑Nicolson method for the 1‑D diffusion equation with decay and source terms

$$
\frac{\partial u}{\partial t} = D \frac{\partial^2 u}{\partial x^2} - k u + S(x)
$$

is discretized on a uniform grid with spacing $\Delta x$ and time step $\Delta t$. Let $u_i^n$ denote the concentration at grid point $i$ and time level $n$. The scheme is
...
Grouping ${\color{blue}\text{explicit}}$ and ${\color{red}\text{implicit}}$ terms gives:

$$
- \frac{\lambda}{2} u_{i-1}^{n+1} + (1 + \lambda + \beta) u_i^{n+1} - \frac{\lambda}{2} u_{i+1}^{n+1} =
\frac{\lambda}{2}{\color{blue} u_{i-1}^{n}} + (1 - \lambda - \beta){\color{blue} u_i^{n}} + \frac{\lambda}{2}{\color{blue} u_{i+1}^{n}} + \Delta t \left( O_i^{n+1} + I_i^{n+1} \right),
$$
