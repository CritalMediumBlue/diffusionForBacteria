...
test("zero iterations leaves the field unchanged", () => {
    const D = 2,
        k = 0.01,
        dt = 0.1;
    const u0 = new Float64Array(length).map((_, i) =>
        Math.cos((2 * Math.PI * (i + 0.5) * deltaX) / L)
    );
...
[1, 2, 5].forEach((n) => {
    test(`cosine eigenmode n=${n} decays at the analytical rate`, () => {
        ...
        for (let i = Math.floor(length * 0.2); i <= Math.floor(length * 0.8); i += 20) {
            const expected =
                uniformDecay + decay * Math.cos((n * Math.PI * ((i + 0.5) * deltaX)) / L);
...
test("total mass is conserved with reflective BC (no source/decay)", () => {
    const D = 3,
        k = 0,
        dt = 0.1;
    const u = new Float64Array(length).map((_, i) =>
        i > length / 3 && i < (2 * length) / 3 ? 2.0 : 0.2
    );
    const sourceCounts = new Float64Array(length).fill(0);
    const sinkCounts = new Float64Array(length).fill(0);
    const K_I = k * deltaX;
...
test("symmetric initial condition remains symmetric in time", () => {
    const D = 2,
        ...
        dt = 0.1;
    const u = new Float64Array(length).map(
        (_, i) => 1 + Math.cos((2 * Math.PI * (i + 0.5) * deltaX) / L)
