...
const TestCases = [
	{ diffusionCoefficient: 100, deltaX: 0.5, width: 200, height: 120, decayRate: 0.01, deltaT: 0.5 },
	{ diffusionCoefficient: 100, deltaX: 0.25, width: 400, height: 240, decayRate: 0.01, deltaT: 0.5 },
	{ diffusionCoefficient: 100, deltaX: 2, width: 50, height: 30, decayRate: 0.01, deltaT: 0.5 },
	{ diffusionCoefficient: 100, deltaX: 0.80, width: 125, height: 75, decayRate: 0.01, deltaT: 0.5 },
	{ diffusionCoefficient: 100, deltaX: 0.4, width: 250, height: 150, decayRate: 0.01, deltaT: 0.5 },
];
...
function sampleAtDefaultGrid(
	grid, testWidth, testHeight, testDeltaX,
	defaultWidth, defaultHeight, defaultDeltaX
) {
...
describe("Numerical Solution with Varying space resolution vs Default Case of dx=1", () => {
	...
	const physicalSources = generatePhysicalSources(
		defaultCase.width, defaultCase.height, defaultCase.deltaX, 0.02, rng
	);

	// 2) Solve the reference (default) case once
	const defaultSources = mapSourcesToGrid(
		physicalSources, defaultCase.width, defaultCase.height,
		defaultCase.deltaX, defaultCase.deltaX          // scale = 1
	);
	const defaultSinks = new Float64Array(defaultCase.width * defaultCase.height).fill(defaultCase.decayRate);
	const defaultSolution = solveToSteadyState(
		defaultCase.width, defaultCase.height, defaultCase.diffusionCoefficient,
		defaultCase.deltaX, defaultCase.deltaT, defaultSources, defaultSinks
	);
	...
	test.each(TestCases)(
		...
		({ diffusionCoefficient, deltaX, width: WIDTH, height: HEIGHT, decayRate, deltaT }) => {
			...
			const sources = mapSourcesToGrid(
				physicalSources, WIDTH, HEIGHT, deltaX, defaultCase.deltaX
			);
			const sinks = new Float64Array(WIDTH * HEIGHT).fill(decayRate);

			// ---- Act ----
			const testSolution = solveToSteadyState(
				WIDTH, HEIGHT, diffusionCoefficient, deltaX, deltaT, sources, sinks
			);

			// Interpolate test solution onto the default grid for comparison
			const sampledTest = sampleAtDefaultGrid(
				testSolution, WIDTH, HEIGHT, deltaX,
				defaultCase.width, defaultCase.height, defaultCase.deltaX
			);
			...
			for (let i = 0; i < N; i++) {
				if (defaultSources[i] === 0) { // skip source cells (sharp peaks)
